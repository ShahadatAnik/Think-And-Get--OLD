...
app.get("/user/verify_login_shop/", (req, res) => {
    //console.log("verify login")
    const email = req.query.email;
    const password = req.query.password;
    console.log("data  "+email+" "+password)
    //console.log(email, password)
    const get_user = "select id,password from shoppers where email = ?";
    db.query(get_user, [email], (err, result) => {
        //console.log(result)
        if (result.length === 0) {
            console.log("No user found");
...
app.get("/user/forgot_password_shop/", (req, res) => {
    //console.log("verify login")
    const email = req.query.email;
    const code = req.query.code;
    //console.log("data  "+email+" "+code)
    const get_user = "select id from shoppers where email = ?";
    db.query(get_user, [email], (err, result) => {
        //console.log(result)
        if (result.length === 0) {
            console.log("No user found");
...
app.post("/user/create_shop", (req, res) => {
    ...
    const shop_type = req.body.shop_type;
    const email = req.body.email;
    const password = req.body.password;
    const phone = req.body.phone;
    //console.log(shop_name, email, password, phone)
    const create_user =
        "INSERT INTO shoppers (name, email, phone, password, shop_location, address, shop_type) VALUES (?, ?, ?, ?, ?, ?, ?);";
    db.query(
        create_user,
        [shop_name, email, phone, password, shop_location, shop_address, shop_type],
        (err, result) => {
            res.send(result);
            if (err) {
                console.log(err);
...
app.get("/user/reset_password_shop", (req, res) => {
    const email = req.query.email;
    const password = req.query.password;
    //console.log(email, password);
    const update_password = "UPDATE shoppers SET password = ? WHERE email = ?";
    db.query(update_password, [password, email], (err, result) => {
        res.send("Password updated");
        if (err) {
            console.log(err);
